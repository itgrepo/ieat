...
const oneTimeTokens = new Map(); // authCode -> jwt
const xmlParserMiddleware = express_1.default.text({
...
router.post('/soap/pmis/token', xmlParserMiddleware, async (req, res) => {
...
router.post('/soap/pmis', xmlParserMiddleware, async (req, res) => {
    res.set('Content-Type', 'text/xml');
    try {
        const rawXml = req.body;
        if (!rawXml) {
...
router.post('/soap/sso', xmlParserMiddleware, async (req, res) => {
    res.set('Content-Type', 'text/xml');
    try {
        const rawXml = req.body;
        if (!rawXml) {
        ...
        try {
            ...
            audit_logger_1.AuditLogger.log('SSO_SOAP_TOKEN_VALIDATION_SUCCESS', {
    ...
    <ValidateTokenResponse xmlns="http://sso.ieat.go.th/">
      <ValidateTokenResult>
        ...
        <department>${decoded.department}</department>
            ...
            audit_logger_1.AuditLogger.log('SSO_SOAP_TOKEN_VALIDATION_FAILED', { error: jwtErr instanceof Error ? jwtErr.message : String(jwtErr) });
            const responseXml = `<?xml version="1.0" encoding="utf-8"?>
<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/">
  <soapenv:Body>
    <ValidateTokenResponse xmlns="http://sso.ieat.go.th/">
...
router.post('/external/pmis/auth/token', async (req, res) => {
...
router.post('/external/pmis/validate-user', async (req, res) => {
    ...
    try {
        ...
        if (pmisUser && pmisUser.enabled === 1) {
            audit_logger_1.AuditLogger.log('PMIS_EXTERNAL_VALIDATION_SUCCESS', { username, email: pmisUser.email, role_id: pmisUser.role_id });
            return res.json({
                exists: true,
                user: {
        ...
        audit_logger_1.AuditLogger.log('PMIS_EXTERNAL_VALIDATION_NOT_FOUND', { username });
...
router.post('/sso/mock-login', async (req, res) => {
    ...
    try {
        if (reset) {
            ...
            if (existing) {
                ...
                audit_logger_1.AuditLogger.log('SSO_USER_RESET_SUCCESS', { username, userId: existing.id });
                return res.json({ success: true, message: `Reset user ${username}` });
            }
            return res.json({ success: true, message: `User ${username} not in DB` });
        }
        audit_logger_1.AuditLogger.log('SSO_CHECK_USER_DB_START', { username, providerId });
        ...
        if (ssoUser) {
            ...
            audit_logger_1.AuditLogger.log('SSO_CHECK_USER_DB_FOUND', { username });
        ...
        audit_logger_1.AuditLogger.log('SSO_USER_DISCOVERY_START', { username });
        ...
        if (!isDiscovered) {
            ...
            audit_logger_1.AuditLogger.log('SSO_USER_DISCOVERY_FAILED', { username, trace, pmisResponse: pmisUserInfo });
...
const handleIssueToken = async (req, res) => {
    ...
    try {
        ...
        if (userId) {
            ...
            if (parts.length === 2 && parts[0].toLowerCase() === 'bearer') {
                ...
                try {
                    const decoded = jsonwebtoken_1.default.verify(ssoToken, JWT_SECRET);
        ...
        audit_logger_1.AuditLogger.log('SSO_TOKEN_ISSUED', { username: user.username, applicationId });
...
router.post('/sso/validate-token', async (req, res) => {
    let { token } = req.body;
    if (!token) {
    ...
    try {
        const decoded = jsonwebtoken_1.default.verify(token, JWT_SECRET);
        audit_logger_1.AuditLogger.log('SSO_TOKEN_VALIDATION_SUCCESS', {
            username: decoded.username,
            applicationId: decoded.applicationId
        });
        return res.json({
        ...
        audit_logger_1.AuditLogger.log('SSO_TOKEN_VALIDATION_FAILED', { error: error instanceof Error ? error.message : String(error) });
        return res.status(401).json({
            valid: false,
...
async function getRolesFromPmis(username) {
    try {
        // 1. GetToken from PMIS
        const tokenUrl = 'https://pmis-uat-phase2.wisdomcloud.net/api/v1/auth/token/service';
        const tokenResponse = await axios_1.default.get(tokenUrl, {
...
const handleValidateToken = async (req, res) => {
    ...
    try {
        ...
        audit_logger_1.AuditLogger.log('SSO_TOKEN_VALIDATION_FAILED', {
...
router.get(['/auth/sso-validate', '/v1/auth/sso-validate'], async (req, res) => {
    ...
    try {
        const decoded = jsonwebtoken_1.default.verify(token, JWT_SECRET);
        audit_logger_1.AuditLogger.log('SSO_VALIDATE_EXTERNAL', {
            username: decoded.username,
            applicationId: decoded.applicationId
        });
        ...
        const pmisResponse = await axios_1.default.post(pmisLoginUrl, {
...
router.post('/v1/auth/sso/vtms', async (req, res) => {
    const { user } = req.body;
    if (!user) {
